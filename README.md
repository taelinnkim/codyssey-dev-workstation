...
## 18. Docker 핵심 개념 및 운영 시 고려사항

### 18-1. 이미지와 컨테이너의 차이 — 이미지 불변성

Docker **이미지(image)** 는 애플리케이션 실행에 필요한 파일과 설정을 담은 읽기 전용 템플릿이다. 한 번 만들어진 이미지 자체는 실행 중에 변경되지 않는 **불변(immutable)** 형태로 취급한다. 내용을 변경해야 할 경우 기존 이미지를 직접 수정하는 것이 아니라 Dockerfile이나 소스 파일을 수정한 뒤 새로운 이미지를 다시 빌드한다.

반면 **컨테이너(container)** 는 이미지를 기반으로 실제 실행된 인스턴스이다. 하나의 이미지에서 여러 개의 컨테이너를 만들 수 있으며, 각 컨테이너에는 실행 중 발생하는 변경 사항을 기록하는 별도의 writable layer가 존재한다. 따라서 컨테이너를 삭제하면 이 계층의 데이터는 사라질 수 있으므로 영구적으로 보관해야 하는 데이터는 Docker volume이나 bind mount 등에 저장해야 한다.

이번 실습에서는 `codyssey-web` 이미지를 빌드한 뒤 이를 이용해 `codyssey-web-container`를 실행함으로써 이미지가 실행 템플릿이고 컨테이너가 실제 실행 단위라는 차이를 확인했다.

### 18-2. 포트 매핑과 보안

Docker 컨테이너는 Linux의 namespace 등을 이용해 프로세스와 네트워크 환경을 호스트와 격리한다. 따라서 컨테이너 내부 웹 서버의 80번 포트는 별도의 포트 매핑 없이는 호스트 브라우저에서 직접 접근할 수 없다.

이번 실습에서는 다음과 같이 호스트 포트와 컨테이너 포트를 연결했다.

```bash
docker run -d --name codyssey-web-container -p 8080:80 codyssey-web
```

여기서 `8080`은 호스트 포트이고 `80`은 컨테이너 내부 웹 서버 포트이다.

단, 포트를 호스트에 공개하면 외부 접근 가능성이 생길 수 있으므로 실제 운영 환경에서는 필요한 포트만 노출해야 한다. 또한 호스트 방화벽, 접근 제어 정책, 공개 인터페이스 범위를 함께 확인해야 한다. 로컬 테스트만 필요하다면 불필요하게 모든 네트워크 인터페이스에 서비스를 공개하지 않는 것이 안전하다.

### 18-3. 포트 충돌 발생 시 진단 절차

컨테이너 실행 시 `port is already allocated` 또는 `address already in use`와 같은 오류가 발생하면 해당 호스트 포트를 다른 프로세스가 이미 사용 중인지 확인해야 한다.

macOS에서는 다음과 같이 확인할 수 있다.
...
1. `lsof -i :8080`으로 해당 포트를 사용하는 프로세스를 확인한다.
2. Docker 컨테이너가 사용 중이라면 `docker ps`에서 해당 컨테이너를 찾는다.
3. 불필요한 프로세스 또는 컨테이너라면 종료한다.
4. 종료할 수 없는 서비스라면 Docker 실행 시 다른 호스트 포트를 사용한다.

예를 들어 8080 포트를 사용할 수 없다면 다음과 같이 8082로 변경할 수 있다.

```bash
docker run -d --name codyssey-web-container -p 8082:80 codyssey-web
```

즉, **포트 점유 확인 → 점유 프로세스 확인 → 필요 시 종료 → 또는 다른 포트 사용** 순서로 문제를 진단한다.

### 18-4. 데이터 손실 방지와 백업

Docker volume은 컨테이너를 삭제해도 데이터를 유지할 수 있지만, volume 자체가 삭제되거나 호스트에 문제가 발생하면 데이터가 손실될 수 있다. 따라서 중요한 데이터는 volume 사용만으로 끝내지 않고 별도의 백업 전략을 함께 사용하는 것이 안전하다.

예를 들어 Docker volume의 데이터를 호스트 디렉터리나 압축 파일로 정기적으로 복사해 별도로 보관할 수 있다. bind mount를 사용하는 경우에는 호스트에 존재하는 원본 디렉터리를 일반적인 파일 백업 도구나 클라우드 저장소를 통해 백업할 수도 있다.

따라서 데이터 보존 방법은 다음과 같이 구분할 수 있다.

* **Docker volume**: 컨테이너 수명과 데이터를 분리하여 보존
* **Bind mount**: 호스트 파일을 직접 사용하므로 호스트 백업 체계 활용 가능
* **별도 백업**: volume 또는 호스트 데이터를 다른 디스크·저장소에 복사하여 장애나 실수로 인한 데이터 손실에 대비
